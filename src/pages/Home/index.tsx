...
const createCards = () => {
  ...
  componentInfos.forEach((card, index) => {
    cards.push(<PresentationCard name={card.name} raw={card.raw} isRequired={card.isRequired} description={card.description} icon={card.icon} key={`home-card-${index}`}/>);
  });
  return cards;
}