import { ComponentInfo } from "../models/ComponentInfo";

// export const componentCards = [
//   new ComponentInfo(
//     "Gabinete",
//     true,
//     "É a carcaça que abriga todos os outros componentes do computador, fornecendo proteção e organização.",
//     "Gabinete.png",
//   ),
//   new ComponentInfo(
//    "Fonte",
//     true,
//     "É a responsável por fornecer energia para todos os componentes do computador.",
//     "Fonte.png",
//   ),
//   new ComponentInfo(
//     "Placa Mãe",
//     true,
//     "É a placa principal do computador, onde todos os outros componentes são conectados.",
//     "Placa-Mae.png",
//   ),
//   new ComponentInfo(
//     "Processador",
//     true,
    
//   "Memória RAM": {
//     required: true,
//     text: "É a memória de curto prazo do computador, onde os dados são armazenados enquanto estão sendo utilizados."
//   },
//   "Armazenamento": {
//     required: true,
//     text: "É o depósito do computador, onde os dados são armazenados a longo prazo."
//   },
//   "Placa de Vídeo": {
//     required: true,
//     text: "É a responsável por processar as informações gráficas do computador."
//   },
//   "Cooler do Processador": {
//     required: false,
//     text: "É o responsável por manter o processador em uma temperatura adequada."
//   },
//   "Cooler do Gabinete": {
//     required: false,
//     text: "É o responsável por manter o gabinete em uma temperatura adequada."
//   },
// ]

export const componentCards: ComponentInfo[] = [
  {
    name: "Gabinete",
    isRequired: true,
    description: "É a carcaça que abriga todos os outros componentes do computador, fornecendo proteção e organização.",
    icon: "Gabinete.png",
  },
  {
    name: "Fonte",
    isRequired: true,
    description: "É a responsável por fornecer energia para todos os componentes do computador.",
    icon: "Fonte.png",
  },
  {
    name: "Placa Mãe",
    isRequired: true,
    description: "É a placa principal do computador, onde todos os outros componentes são conectados.",
    icon: "Placa-Mae.png",
  },
  {
    name: "Processador",
    isRequired: true,
    description: "É o cérebro do computador, responsável por processar todas as informações.",
    icon: "CPU.png",
  },
  {
    name: "Memória RAM",
    isRequired: true,
    description: "É a memória de curto prazo do computador, onde os dados são armazenados enquanto estão sendo utilizados.",
    icon: "Memoria.png",
  },
  {
    name: "Armazenamento",
    isRequired: true,
    description: "É o depósito do computador, onde os dados são armazenados a longo prazo.",
    icon: "SSD.png",
  },
  {
    name: "Placa de Vídeo",
    isRequired: true,
    description: "É a responsável por processar as informações gráficas do computador.",
    icon: "GPU.png",
  },
  {
    name: "Cooler do Processador",
    isRequired: false,
    description: "É o responsável por manter o processador em uma temperatura adequada.",
    icon: "Cooler.png",
  },
  {
    name: "Cooler do Gabinete",
    isRequired: false,
    description: "É o responsável por manter todo o gabinete em uma temperatura adequada.",
    icon: "Cooler.png",
  },
]